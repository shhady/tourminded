...
export default function GuideCard({ guide, locale }) {
  ...
  // Safely extract data with proper checks to avoid circular references
  const guideName = typeof guide.name === 'object' 
    ? (guide.name[locale] || guide.name.en || 'Guide') 
    : (guide.name || 'Guide');
    
  const guideBio = typeof guide.about === 'object'
    ? (guide.about[locale] || guide.about.en || '') 
    : (guide.about || '');
    
  const imageUrl = guide.profileImage && guide.profileImage.url 
    ? guide.profileImage.url 
    : null;
    
  const guideRating = typeof guide.rating === 'number' ? guide.rating : 0;
  
  const expertiseAreas = Array.isArray(guide.expertise) && guide.expertise.length > 0
    ? guide.expertise.map(exp => exp.area || '')
    : [];
    
  const languages = Array.isArray(guide.languages) && guide.languages.length > 0
    ? guide.languages.map(lang => lang.language || '')
    : [];
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-6 text-white">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-2xl font-bold tracking-tight drop-shadow-md">{guideName}</h2>
          {guideRating > 0 ? (
            <div className="flex items-center bg-yellow-400 text-gray-900 px-2.5 py-1 rounded-full shadow-md">
              ...
              <span className="font-bold">{guideRating.toFixed(1)}</span>
            </div>
          ) : null}
        </div>
        ...
        <p className="text-sm text-white/85 mb-4 line-clamp-2 transition-all duration-300 group-hover:line-clamp-3 drop-shadow-sm">
          {guideBio || 'Professional local guide'}
        </p>
