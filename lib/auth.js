...
export const getCurrentUser = async () => {
  try {
    // Create a timeout promise that resolves with null after 3 seconds
    // This prevents unhandled rejection errors
    const timeoutPromise = new Promise((resolve) => {
      setTimeout(() => {
        console.log('User authentication timed out, continuing as guest');
        resolve(null);
      }, 3000);
    });
    ...
    try {
      ...
      if (authOptions) {
        ...
        if (session?.user) {
          ...
          const userDocPromise = User.findById(session.user.id).select('-password').lean();
    ...
    // Use Promise.race to prevent hanging
    const userDocPromise = User.findById(decoded.id).select('-password').lean();
    const userDoc = await Promise.race([userDocPromise, timeoutPromise]);
